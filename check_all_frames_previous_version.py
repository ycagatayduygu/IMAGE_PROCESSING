...
while True:
    ...
    found_contour = False  # Initialize flag for found contour



    # Draw centroids and numbers
    for i, contour in enumerate(filtered_contours):
        if i == contour_to_track-1:
            moments = cv2.moments(contour)
            if moments["m00"] != 0:
                centroid_x = int(moments["m10"] / moments["m00"])
                centroid_y = int(moments["m01"] / moments["m00"])
                ...
                cv2.putText(frame, str(i + 1), (centroid_x + 5, centroid_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                print(centroid_x, centroid_y)
                found_contour = True  # Set flag to stop searching
    

    out.write(frame)
            
    # Display the frame with the tracked contour
